...
export type { IdentityDivergence, IdentityRegistryEntry } from '../../state/identityDivergence';
 ...
 *
 * @param original - The saved camera.
 * @param edited - The edited camera data.
 ...
 *
 * @param workspace - The workspace slice (`{ animals }`).
 * @param exclude - The camera (or whole animal) being edited. With `id`, only that one camera is
 *   excluded; with `animalId` alone (`id` omitted), the animal's ENTIRE camera catalog is excluded —
 *   mirroring {@link collectDataAcqIdentities}, so a caller comparing a whole copied catalog against
 *   the rest of the workspace isn't tripped by the target's own cameras.
 * @returns The camera-name identity registry.
 */
export function collectCameraIdentities(
  workspace: { animals?: Record<string, Animal> } | null | undefined,
  exclude: { animalId: string; id?: number } | null = null
): IdentityRegistryEntry[] {
  const registry: IdentityRegistryEntry[] = [];
  for (const animal of Object.values((workspace?.animals ?? {}) as Record<string, Animal>)) {
 ...
 *
 * @param workspace - The workspace slice (`{ animals }`).
 * @param exclude - The animal or item being edited.
 * @returns The data-acq-name identity registry.
 */
export function collectDataAcqIdentities(
  workspace: { animals?: Record<string, Animal> } | null | undefined,
  exclude: string | { animalId: string; index?: number } | null = null
): IdentityRegistryEntry[] {
