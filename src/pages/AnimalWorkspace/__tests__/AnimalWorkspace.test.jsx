...
describe('Recording-days pane (hosted by AnimalView at the route)', () => {
  ...
  describe('Calendar', () => {
    it('shows one add-recording-day CTA that opens the calendar', async () => {
      ...
      const addButtons = screen.getAllByRole('button', { name: /add multiple dates/i });
      expect(addButtons).toHaveLength(1);
      const [addButton] = addButtons;
      expect(addButton).toHaveTextContent(/add multiple dates/i);
    ...
    it('hides the calendar when the close button is clicked', async () => {
      const user = userEvent.setup();
      renderPane('testanimal', { testanimal });
      await user.click(screen.getByRole('button', { name: /add multiple dates/i }));
  ...
  describe('Edit Animal Setup link (Task 2.1 — removed)', () => {
    it('does not render an "Edit Animal Setup" link in the day-tab header (its destinations are the setup tabs now)', () => {
      ...
      expect(screen.getByRole('button', { name: /add multiple dates/i })).toHaveTextContent(/add multiple dates/i);
    });
  });
});
