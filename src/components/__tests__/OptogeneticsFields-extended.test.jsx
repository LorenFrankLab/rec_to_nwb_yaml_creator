...
describe('OptogeneticsFields - Extended Coverage', () => {
  ...
  describe('fs_gui_yamls - Advanced State Script Parameters', () => {
    ...
    it('allows entering pure numeric values in all advanced parameter fields', async () => {
      const user = userEvent.setup();
      renderWithProviders(<OptogeneticsFields />, { initialState });
      ...
      // Test pulseLength - pure numeric
      const pulseLengthField = screen.getByLabelText(/Length of Pulse/i);
      await user.clear(pulseLengthField);
      await user.type(pulseLengthField, '5');
      expect(pulseLengthField).toHaveValue(5);

      // Test nPulses - pure numeric
      const nPulsesField = screen.getByLabelText(/Number of Pulses per Train/i);
      await user.clear(nPulsesField);
      await user.type(nPulsesField, '10');
      expect(nPulsesField).toHaveValue(10);

      // Test sequencePeriod - pure numeric
      const sequencePeriodField = screen.getByLabelText(/Sequence Period/i);
      await user.clear(sequencePeriodField);
      await user.type(sequencePeriodField, '100');
      expect(sequencePeriodField).toHaveValue(100);

      // Test nOutputTrains - pure numeric
      const nOutputTrainsField = screen.getByLabelText(/Number of Output Trains/i);
      await user.clear(nOutputTrainsField);
      ...
      expect(trainIntervalField).toHaveValue(1000);
    });

    it('allows entering decimal numeric values in advanced parameter fields', async () => {
      const user = userEvent.setup();
      renderWithProviders(<OptogeneticsFields />, { initialState });
      ...
      const pulseLengthField = screen.getByLabelText(/Length of Pulse/i);
      await user.clear(pulseLengthField);
      await user.type(pulseLengthField, '2.5');
      expect(pulseLengthField).toHaveValue(2.5);

      const trainIntervalField = screen.getByLabelText(/Train Interval/i);
      await user.clear(trainIntervalField);
      await user.type(trainIntervalField, '500.75');
      expect(trainIntervalField).toHaveValue(500.75);
    });
  });
