 ...
 *      epoch edit button + the failed-channels checkbox grid are keyboard-operable (the standing acceptance
...
test.describe('redesign — Day Editor IA layout', () => {
  test('1280x720 renders the focused six-section Day Editor rail', async ({ page }) => {
    ...
    for (const group of ['DAY', 'RECORDING', 'FINISH']) {
...
test.describe('redesign — keyboard operability', () => {
  ...
  test('the failed-channels grid toggles are keyboard-operable checkboxes', async ({ page }) => {
    await seedAndOpen(page, buildConfiguredWorkspaceBlob(), `/#/day/${DAY_ID}`);
    await page.getByRole('button', { name: /^Failed Channels\b/i }).click();
