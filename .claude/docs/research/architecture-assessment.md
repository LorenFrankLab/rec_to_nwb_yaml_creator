...
| --- | --- | --- |
| **No static types** ⏳ PARTIAL | Incremental TS underway (`tsc --noEmit` CI gate live): `io/yaml.ts`, `state/workspaceTypes.ts` + the task-catalog modules, `domain/deviceOverrideMerge.ts`, and the Phase-9a validation leaf modules (`repairRouting`/`geometryProvenance`/`dayOverrideValidation`/`stepStatus`) are `.ts` under `strict`; the `validation/` pure leaf predicates (`paths`/`dandiSubject`/`behavioralEvents`/`taskEpochs`) AND the whole custom business-rules layer (`rules/*` families + the `rulesValidation` composer + the shared `issueTypes.ts` `ValidationIssue`) are now `.ts` under `strict`. Remaining in `validation/`: `index`/`schemaValidation`/`rawShape`/`quickChecks`. Remaining elsewhere: most components, `useWorkspace`, and the legacy form path stay JS (`checkJs:false`) | Rename a domain fn or change an `Animal`/`Day` shape → **silent until a test happens to exercise it.** No safe IDE rename, no null-safety. Tooling agent scored type-safety **2/10** (improving as the pure core converts). |
