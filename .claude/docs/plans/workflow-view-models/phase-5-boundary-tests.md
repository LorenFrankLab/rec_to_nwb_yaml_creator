...
The builder phases (2a–2d) each test their own surface; the command phase tests writes. This phase adds
the **cross-cutting scenario matrix** at the view-model + command layer — the cases that span surfaces or
exercise the nasty states — so the UI experiments in [phase-6](phase-6-workflow-ui.md) have a safety net
that stays green while the rendering shape changes. Net-new tests only; no source change.

**Inputs to read first:**

- All four builders (`src/viewModels/*ViewModel.ts`) and the commands (`src/viewModels/commands/`).
...
- Create `src/viewModels/__tests__/scenarios.test.ts` (or one file per scenario family) driving the
  builders/commands through the matrix below. Each scenario is built once as a fixture and asserted
  across the relevant builders (so e.g. an "animal setup issue affecting day export" case is checked in
  BOTH `animalViewModel` rings AND `dayEditorViewModel` export gate — proving the surfaces agree).
...
least one builder, every cross-surface row asserts agreement between the surfaces named, and the full
severity-mapping table is covered. `npx vitest run` + `baselines` green.
...
(not the same builder twice); tests are non-trivial (real fixtures, real comparisons — see
`testing-anti-patterns`); fixtures are shared, not copy-pasted; no source logic changed in test files.
