...
- `src/state/workspaceTransitions.js:348-399` — `applyDayUpdates`: **writes only an explicit allow-list of day keys** (session/technical/state/deviceOverrides/tasks/behavioral_events/associated_files/associated_video_files/fs_gui_yamls/keywords/configurationVersion). A new `cameras_used` write is silently dropped unless added here — confirm the exact allow-list and add the key.

## Tasks

- **Allow-list `cameras_used` in `applyDayUpdates`** (`workspaceTransitions.js:348-399`): add a `cameras_used` branch to the day-update allow-list, else the checklist's `onFieldUpdate('cameras_used', …)` write is silently no-op'd and the feature never persists. (Leave `cameras_used` absent in `createDayRecord` — undefined is the baseline-safe default.)
...
| `cameras_used` actually persists | `updateDay(dayId, { cameras_used: [1] })` is written through (NOT dropped by the allow-list) and survives a re-read |
| baselines | byte-identical (no fixture sets `cameras_used`) |
