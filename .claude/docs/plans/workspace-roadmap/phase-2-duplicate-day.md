...
- `src/pages/AnimalWorkspace/RecordingDaysTab.jsx` — the day-row list; the existing "Delete recording day …" control (`:511-513`) is the sibling for the new "Duplicate day…" item.
...
- **`duplicateDay(sourceDayId, newDate)` action** in `useWorkspace.js`: resolve `source = prev.days[sourceDayId]` (throw if absent), `animalId = source.animalId`, `dayId = generateDayId(animalId, newDate)` (throw if exists). Build with `createDayRecord(animal, animalId, dayId, newDate, { session_id: \`${animalId}_${newDate.replace(/-/g,'')}\`, session_description: source.session?.session_description ?? '' }, now, source)`, then **override the pinned version to the source's** AND **carry the source's `deviceOverrides`** (bad channels) so the duplicate matches: return the day as `{ ...built, configurationVersion: source.configurationVersion, deviceOverrides: source.deviceOverrides ? structuredClone(source.deviceOverrides) : built.deviceOverrides }`. A duplicate is the SAME config version as the source by construction, so carrying its `deviceOverrides.bad_channels` is always safe — **no version guard needed** (that guard is only for the carry-FORWARD/new-day path in phase 5, where the new day pins `latestVersion` which may differ). Append `dayId` to the animal's `days`.
...
- The version-GUARDED carry-forward for NEW-day creation — that's phase 5 (a new day pins `latestVersion`, so its bad-channel carry is guarded). Duplicate carries `deviceOverrides` directly (same config version, no guard) — see the task above.
...
| `duplicateDay` carries bad channels | new `deviceOverrides.bad_channels` equal source's (cloned, not aliased) — even when the source pins a non-latest version |
