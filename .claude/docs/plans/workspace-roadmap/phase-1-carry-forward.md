...
- **CHANGELOG** (`docs/REFACTOR_CHANGELOG.md`): one entry — carry-forward day creation; day-owned fields carried (list them); default-on toggle / opt-out; merge-neutral, 125 baselines byte-identical; trailing-optional params (no breaking change).
...
| baselines | `npx vitest run baselines` → 125 byte-identical |
