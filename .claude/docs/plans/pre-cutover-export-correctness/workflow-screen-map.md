...
| Behavioral-events ownership | **done (Task 6 ✓)**: the animal-level section is reframed as a `Behavioral Events / DIO library` (reusable templates, NOT exported until used on a day) — the false "inherited by all recording days" copy is gone; the day's `BehavioralEventsDisplay` adds a per-event **`Use on this day`** action that copies a library event into the exported day list, and flags a duplicate **description** among exported day events inline (the downstream hard `raise ValueError`, also export-gated by `duplicate_behavioral_event_description`) | — | Task 6 — **done** |
| Opto setup vs. per-day protocol | **done (Task 7 ✓)**: the two-layer model is now explicit. Animal Editor opto step is `Optogenetics Setup` with an intro naming it the animal's **implanted setup** (set once); the day's FsGUI section is `Optogenetics run this day (FsGUI protocols)` — OPTIONAL, epoch-scoped — and its empty state reads **"No optogenetic stimulation recorded for this day — a normal, valid state"** rather than the neutral "No FsGUI protocols added". A `rulesValidation` test pins the invariant: an opto-implanted animal with an opto-free day (empty `fs_gui_yamls`) raises no opto error | the Export/Validation **summary** still reports opto "on" from implant metadata for an opto-free day — see the `Opto preflight` row (Task 10) | Task 7 — **done** (editor split); summary reporting is Task 10 |
| Validation/Export issue copy | **done (Task 9 ✓)**: each issue now carries an ownership-pattern hint (`IssueOwnershipHint`) naming the safe next action ("Fix shared animal setup", "Pin or fix the configuration version", "Override this day's technical value", "Select the item used on this day", "Repair recovered data", …) plus an emphasized "Affects more than this day" cue when the repair's surface-aware scope reaches past the day. Rendered identically in `ValidationStep` and the Export-preflight `RepairActions` list. Copy is read verbatim from the single ownership descriptor; grouping (workflow categories) and routing (`repairTargetForIssue`) are unchanged | the status/preflight SUMMARIES (workspace checklist, version bar, validation summary, batch/single preflight) still need the same vocabulary | Task 9 — **done**; summaries are Task 10 |
...
| Day tasks/epochs heading | **legibility done (Task 5c ✓)**: the tasks table now shows the per-task **Room** (`task_environment`) alongside Cameras and Epochs (the room·cameras·epochs mapping), a `duplicate_task_epoch` collision is surfaced inline as a ❌ prevented error on both colliding tasks (each epoch belongs to exactly one task), and the copy says cameras are selected from the animal's shared catalog. Camera empty states offer `Set Up Cameras` and explain the shared-catalog model | the `Tasks & Epochs` heading itself is fine | Task 5 — **done** |
...
| Opto preflight / batch status | **done (Task 10 ✓)**: the single-day Export preflight and the batch Export preflight now report the DAY-PROTOCOL state via the shared `describeDayOptoState` — "No optogenetics / Implanted, no stimulation this day / Stimulation on epoch(s) …" — instead of a binary On/Off derived from the implant. The two summaries read the same helper, so an opto-implanted animal with an opto-free day reads "implanted, no stimulation", matching the Task 7 editor copy | — | Task 10 — **done** |
| Validation Summary scan fields | **done (Task 10 ✓)**: each readable day row carries a `Setup` column — pinned configuration version (`config vN (historical)`), camera count, and day-protocol opto state — so days are comparable before opening each editor (batch-row scan contract). Unreadable/missing/wrong-owner rows show `—` | the Workspace day rows still show only state chips (no config/camera/opto scan) — a smaller catch-up if desired | Task 10 — **done** (Validation Summary); Workspace rows optional |
| Lifecycle cleanup | **done (Task 8 ✓)**: discoverable secondary/destructive `Delete animal…` (danger zone at the foot of the selected-animal section, set apart from the primary setup/export actions) and `Delete day…` (on each ordinary day row, OUTSIDE the navigation link). Both use a destructive `alertdialog` confirm that names the animal/day + cascade count + consequence; when a day was validated/exported the confirm adds "workspace metadata only — does not delete already-downloaded YAML / NWB / DANDI / Spyglass". The animal confirm counts only the `OK` (index-resident, owned) days the store's `deleteAnimal` actually removes — it excludes wrong-owner records (preserved) and recovered-unlinked records (not in the index → survive as orphans), naming both so the count can't over-promise. Recovered/wrong-owner rows keep their existing repair paths (no delete button) | — | Task 8 — **done** |
