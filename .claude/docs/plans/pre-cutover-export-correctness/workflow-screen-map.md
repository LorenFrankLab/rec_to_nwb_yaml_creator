...
| Animal editor steps | ~~`Hardware Config`~~ → **`Recording System, Cameras & DIO`** (Task 2a ✓); `Electrode Groups`, `Channel Maps`, `Optogenetics` unchanged | `Electrodes & Ephys`, `Channel Maps`, `Optogenetics Setup` relabels still open | Task 2 (Hardware Config relabel **done 2a**; other step relabels pending) |
...
| Data-acq section | ~~`Data Acquisition Device`~~ → **`Recording System`** with "editing this affects all recording days" copy (Task 2a ✓) | rig constants `raw_data_to_volts`/`times_period_multiplier` surfacing here is Task 3/4 | Task 2 heading **done 2a**; Task 3/4 for the constants |
