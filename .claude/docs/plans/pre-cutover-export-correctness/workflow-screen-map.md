...
| Animal Setup versioning subtitle | claims cameras & data acquisition "apply to the latest configuration" and historical days keep theirs ([AnimalEditorStepper.jsx:793](../../../../src/pages/AnimalEditor/AnimalEditorStepper.jsx)) — **only true for ephys config today**; export still reads live `animal.cameras` and `animal.devices.data_acq_device`, so this OVERPROMISES per-day versioning | cameras: true only after the day-used camera export binding lands (Task 5); data-acq: option B shared-with-blast-radius, no per-day version (Task 3). Until then the subtitle must not promise per-day versioning for cameras/data-acq | Tasks 3/5 (export-binding-dependent; do not "fix" the copy before the binding or it asserts a different false claim) |
