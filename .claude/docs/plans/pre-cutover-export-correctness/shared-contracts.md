...
| `cameras` | **Current pre-8.7 behavior:** `animal.cameras` — each `{id, camera_name, manufacturer, model, lens, meters_per_pixel}` with `lens` **required** (schema `:697`). Hardware Config add/edit/delete writes the animal catalog here (phase 3). **Required Phase 8.7 resolution:** `resolveDayCameraUsage(animal, day)` is the export binding, so the exported `cameras` list contains only cameras referenced by that day's tasks/videos/FsGUI rows. Keeping all `animal.cameras` is not an alternate successful cutover path; if the export-bridge change is deferred, mark Phase 8.7 incomplete and warn that camera catalog changes affect all day exports. |
