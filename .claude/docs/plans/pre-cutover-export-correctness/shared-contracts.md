...
| `cameras` | **DONE (Phase 8.7 Task 5a ✓):** `resolveDayCameraUsage(animal, day)` ([src/state/cameraUsage.js](../../../../src/state/cameraUsage.js)) is the export binding — `mergeDayMetadata` emits only the cameras referenced by that day's `tasks[].camera_id` / `associated_video_files[].camera_id` / `fs_gui_yamls[].camera_id`, FILTERED from the full catalog objects (each keeps `lens` and all schema-required fields), preserving `cameras: []` for a zero-camera day. Byte-identical on all 125 golden/legacy baselines (every fixture references all its cameras); only a real-data day that left a catalog camera unused changes, in the correct direction. trodes_to_nwb resolves cameras by `id` not list position, and the `dangling_camera_ref` rule still gates a referenced-but-absent id. |
