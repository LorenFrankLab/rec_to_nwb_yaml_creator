...
### The `day.technical.*` export fields

These all live in `day.technical` and export from there, but they are NOT all "copied defaults":
only `raw_data_to_volts` / `times_period_multiplier` are seeded from `animal.technicalDefaults`
(`setup_default_to_day`). `default_header_file_path` and `units` are seeded empty/undefined at day
creation and are plain `day_fact`s — same container, different ownership (this is exactly the
distinction Task 4 must render, per [shared-contracts.md](shared-contracts.md#export-resolution-source-of-truth-contract)).
...
| `units` (`analog`, `behavioral_events` unit strings) | `day_fact` | **Day-specific**, NOT a copied default: day creation seeds `units: undefined` (there is no `animal.technicalDefaults.units`), unlike the rig constants above. Lives in `day.technical` but follows `default_header_file_path`, not the rig constants. The exporter **deletes the key when empty** (schema rejects present-but-empty) and must keep doing so | `day.technical.units` | merge → `units` (omitted when empty) | Day Editor → Setup/Technical | `day` / devices | Recording units (this day) | `This day only` | Fix this day's units | Treating units as a recording-system default with day-wide blast radius; emitting an empty `units` (byte/schema regression) | Day Technical section | `workflowOwnership` units path test (day-fact + electrode-group-`units` ordering guard); merge `units` delete-when-empty (baseline); Task 4 |
