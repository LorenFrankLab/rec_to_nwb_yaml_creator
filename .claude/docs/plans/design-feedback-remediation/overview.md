...
- `src/index.css:26-27` (`.primary-nav { position: relative; z-index: 1 }`) vs `src/App.scss:22` (`.home-region`, no z-index; desktop `position: fixed` ~`:573`) — the F3 occlusion (DOM siblings: `AppLayout.jsx:248` vs `:282`); **Phase 3** fixes via the z-index scale. *Browser-verify before/after.*
...
- `src/validation/rulesValidation.js` — has the epoch-ownership rule (`duplicateTaskEpochs` ~`:752`); a **`task_name` uniqueness rule does NOT exist** today — Phase 8 adds it.
...
- `src/components/CalendarDayCreator/CalendarDay.jsx:93` (`tabIndex={isToday?0:-1}`), `CalendarGrid.jsx:100` (42 cells in one `role="row"`; built by `getCalendarDays` ~`:50`) — **Phase 10** a11y (#7/#8).
