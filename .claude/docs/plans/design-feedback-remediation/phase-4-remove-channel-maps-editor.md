...
- `src/__tests__/integration/axe-a11y.test.jsx`, `src/__tests__/integration/migrated-dialogs-modal-a11y.test.jsx` (import `ChannelMapEditor`), `src/pages/AnimalView/__tests__/AnimalView.navDiscardGuard.test.jsx` + `AnimalView.ephysTabs.test.jsx` (reference `ChannelMapsContainer`), and the `CROSS_PAGE_ALLOWLIST` entry in `src/__tests__/architectureBoundaries.guard.test.js:51` — all break or go stale on deletion. **The Task #1 grep output is authoritative; these are examples, not the full list.**
