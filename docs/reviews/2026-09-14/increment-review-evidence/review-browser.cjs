...
const fs = require('fs');
const base=process.env.REVIEW_BASE_URL || 'http://127.0.0.1:3012';
...
(async()=>{
 ...
 try {
  ...
  const readerInput=b.locator('#session-description');
  ...
  await a.locator('#session-description').fill('Writer saved observations');await save(a);
  ...
  result.readonly.storedValue=await b.evaluate(()=>JSON.parse(localStorage.getItem('rec_to_nwb_workspace_v1')).workspace.days['remy-2023-06-22'].session.session_description);
  await c.close();
  ...
  await a.locator('#session-description').fill('Unsaved valuable observation');await save(a);
  await b.getByRole('button',{name:'Edit in this tab instead'}).click();await role(b,'writer');await role(a,'reader');
  await b.locator('#session-description').fill('New writer adds more data');await save(b);
  await expect(a.locator('#session-description')).toHaveValue('New writer adds more data');
  result.failedHandover={oldWriterValue:await a.locator('#session-description').inputValue(),newWriterValue:await b.locator('#session-description').inputValue()};
  await c.close();
  ...
  await c.close();
  console.log(JSON.stringify(result,null,2));fs.writeFileSync('/tmp/rec-yaml-review-evidence.json',JSON.stringify(result,null,2));
