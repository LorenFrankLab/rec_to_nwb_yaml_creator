...
## Workflow/UX/architecture review response (docs/reviews/2026-09-15/WORKFLOW_UX_ARCHITECTURE_REVIEW.md, branch `fix/workflow-review-findings`): F1–F7 + arch items 2, 3, 5 fixed

Executed as a nine-task plan (`.claude/docs/plans/workflow-review-fixes/PLAN.md`): Opus implementers, a Fable review per task with fix rounds, and a Fable whole-branch review with one final fix wave. Deferred by ruling: F8 (quarantined imports), arch items 1, 4, 6, 8 and multi-browser Playwright.

| Finding | Fix | Failing-first test |
| --- | --- | --- |
| F4 Save draft requires weight/DOB | Weight and DOB optional in every wizard commit path; `createAnimal` no longer seeds `weight: 100`; an explicit `undefined` in a `subject` payload deletes the key (`mergeSubject`), so clearing a DOB in the wizard or profile dialog leaves it absent, never `''`; missing DOB is a blocking export issue with a friendly message routed to the animal profile. | `CreateAnimalWizard.test.jsx` Save draft without weight/DOB; `AnimalProfileDialog.test.jsx` clearing DOB removes it |
| F2 Copy lowercases and traps an invalid id | One `validateSubjectId` rule (case preserved, underscores/slashes rejected, case-insensitive collision naming the existing animal) used by the wizard and Copy-from-animal before `createAnimal`. Import keeps its own charset gate (historical files stay importable; the export gate rejects the id). | `animalCreation.test.js` `ReviewCase` kept / `Review_Rat` rejected; `CopyFromAnimal.test.jsx` |
| F3 Task-type edits rewrite history | `TaskInstance` carries per-day `task_environment`/`camera_id` overrides applied by `resolveTaskInstances`; the epoch editor edits them for this day; a type-default edit with referencing days asks "keep earlier days as recorded (pin)" (default) or "also correct N days"; derive/import keep SC38-style environment differences as overrides; carry-forward starts a new day from the type default. | `taskCatalog.test.js` overrides/derive/pin; `TaskTypesContainer.test.jsx` scope choice + next day on new default; `EpochsTab.test.jsx` per-day editor |
| F1 Import collapses camera calibrations | `planImport` reports structured `cameraConflicts`; default `split` gives each calibration its own camera (`overhead_camera_20230623`), `unify` is an explicit choice that never rewrites an existing animal's row; re-import reroutes onto an existing split row (idempotent); preview fieldset shows both values with files/dates; result lists what was written or discarded. | `planImport.test.js` split/unify/reroute; `applyImportPlan.test.js` each day exports its own calibration |
| F6 "Ready" beside "statescripts missing"; folder field hidden | Inline data-folder input beside the generate buttons; one `statescriptState` (linked / expected / not expected, carry-forward expectation rule in `src/domain/statescriptExpectation.ts`) drives the amber chip, the filter, the row cell and the bulk generator; "Ready to export · N warnings to review". | `statescriptExpectation.test.ts`; `epochGeneratedFiles.test.ts` not-expected rows never generated |
| F7 No readable review before download | Shared `EffectiveDayReview` rendered on Fix & Export; preflight gains "Weight & team" (names joined with `;`) and per-task `name (epochs) — environment`. | `preflightSummary.test.js`; `ExportPreview.test.jsx` |
| F5 + arch 3 Tab skips day sections; 2.46:1 badge; 15 px footer links | `tabIndex=-1` removed; nine sub-AA warning-text rules in six stylesheets moved to `--color-warning` with a contrast guard test measured on the amber panel; 24 px footer targets. | `DayEditorSectionNav.test.jsx` Tab reaches every section; `contrast.test.js` |
| Arch 2 `ElectrodeGroup` coordinate types | `targeted_location: string`, `targeted_x/y/z: number`, `units: string`; paper-over casts removed. **Found on the way:** the v3 electrode-group modal labelled `targeted_x` "AP" and `targeted_y` "ML" — the schema says x = ML, y = AP, and Spyglass derives the hemisphere from `targeted_x`. Labels corrected (legacy form was already right); CHANGELOG warns that v3-entered coordinates may be transposed. | `ElectrodeGroupModal.test.jsx` label→field association |
| Arch 5 One 1.31 MB bundle | Every route `React.lazy` behind one `Suspense`; entry 1,331 → 429 kB; focus moves to the routed page on reveal (`RouteFocus` inside the boundary). | `AppLayout.lazyRoutes.test.jsx` |
| Final review: camera blast radius missed instance/type refs; probe summary hard-coded `mm` | `findCameraAffectedDays` reads effective tasks via the exported `resolveDayTasks`; summary uses `group.units`. | `cameraUsage.test.js`; `animalViewModel.test.ts` |

Verification at `6d7aa64b`: `npx vitest run` **387 files / 5,644 tests pass**; golden baselines byte-identical (no fixture regenerated); typecheck, eslint, build exit 0; stylelint warnings only; Playwright workspace/legacy specs pass, the same 7 pre-existing legacy visual baselines fail (regenerate deliberately — the footer target-size change may shift them further).

Follow-ups (from the reviews, not on this branch): `OpticalFiber`/`VirusInjection` types still declare `coordinates: number[]` (schema uses `ap_in_mm`…; no read site); three "context value" predicates to consolidate beside `TASK_CONTEXT_FIELDS`; duplicate friendly-message tables; profile-dialog sex `''` option should map to `U`; a route-scoped error boundary for a chunk 404 after redeploy; non-text orange accents below 3:1; `isSleepTaskName` (`\bsleep\b`) against real corpus task names; a nudge for labs that run a task in a non-default room on consecutive days (carry-forward deliberately resets to the type default).
