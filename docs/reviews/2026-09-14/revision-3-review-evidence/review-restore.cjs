...
(async () => {
 ...
 try {
  // Cancel remains available after starting a restore; the continuation survives closing the panel.
  {
   ...
   await expect.poll(() => p.evaluate(k=>JSON.parse(localStorage.getItem(k)).workspace.days['remy-2023-06-22'].session.weight,key)).toBe(480);
   result.cancelledRestore.storedAfterCancelledRestoreCompletes = 480;
 ...
 } finally {await browser.close();console.log(JSON.stringify(result,null,2));fs.writeFileSync('/tmp/revision3-restore.json',JSON.stringify(result,null,2)+'\n');}
