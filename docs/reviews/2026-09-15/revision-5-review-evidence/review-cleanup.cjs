 ...
 try {
  ...
  const backup=await p.evaluate(async k=>{
   ...
   for(const day of Object.values(ws.days)){
    ...
    const artifact={filename:exported.filename,yaml:exported.yaml,exportedAt:'2023-06-22T00:00:00Z'};
    day.exportReceipt={filename:artifact.filename,exportedAt:artifact.exportedAt,contentHash:exported.hash,yamlStored:true,appVersion:'test',schemaVersion:4};
    await putBlob('receipt:'+day.id,artifact);
   }
  ...
  await p.evaluate(()=>{
   window.__cleanupHeld=[];
   const originalDelete=IDBObjectStore.prototype.delete;
   IDBObjectStore.prototype.delete=function(name){if(name==='receipt:remy-2023-06-21')this.transaction.__holdCleanup=true;return originalDelete.call(this,name);};
   const originalTx=IDBDatabase.prototype.transaction;
   IDBDatabase.prototype.transaction=function(...args){const tx=originalTx.apply(this,args);Object.defineProperty(tx,'oncomplete',{configurable:true,set(fn){tx.addEventListener('complete',e=>{if(tx.__holdCleanup)window.__cleanupHeld.push(()=>fn.call(tx,e));else fn.call(tx,e);});}});return tx;};
  });
  ...
  await expect.poll(()=>p.evaluate(async()=>{
   const {getBlob}=await import('/src/state/blobStore.ts');return (await getBlob('receipt:remy-2023-06-22'))??null;
  })).toBeNull();
  await p.reload();
  result.cleanup.afterReload=await p.evaluate(async k=>{
   const {getBlob}=await import('/src/state/blobStore.ts');const {receiptYamlKey}=await import('/src/domain/exportReceipt.ts');
   const receipt=JSON.parse(localStorage.getItem(k)).workspace.days['remy-2023-06-22'].exportReceipt;
   return {yamlStored:receipt.yamlStored,artifact:(await getBlob(receiptYamlKey('remy-2023-06-22',receipt)))??null};
  },key);
  await p.getByLabel('Weight measured today (grams)').fill('499');await p.keyboard.press('Control+s');
  await p.getByRole('button',{name:'Export',exact:true}).click();
  await expect(p.getByText(/previous download’s bytes are not available/)).toBeVisible();
  result.cleanup.comparisonUnavailable=true;
  await c.close();
 }finally{await browser.close();console.log(JSON.stringify(result,null,2));fs.writeFileSync('/tmp/revision5-cleanup.json',JSON.stringify(result,null,2)+'\n');}
