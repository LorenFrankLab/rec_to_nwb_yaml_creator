...
| `data_acq_device` | **array**, iterated (`for i, acq in enumerate(...)`) → one `DataAcqDevice(name=f"dataacq_device{i}", system, amplifier, adc_circuit)` per entry, all added to the session; reads `system`/`amplifier`/`adc_circuit` | missing any of those three → `KeyError` | **`name` is NOT read** — device named `dataacq_device{i}`. A single object instead of an array breaks iteration. **Multiple entries do NOT raise, but are scientifically meaningless: each attaches to the one session with NO link to which electrodes/data it recorded.** A .rec session is recorded by ONE acquisition system, so the meaningful unit is **one device per YAML/session**; an animal recorded on different systems over time is modelled by **different days/YAMLs, each with its own single device**, not multiple devices in one YAML |
